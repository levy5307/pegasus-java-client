...
/**
 * @author qinzuoyan
 *     <p>This class provides a client tool to access pegasus data.
 */
public class PegasusCli {
  ...
  public static void usage() {
    ...
    System.out.println(
        "         - copy_data <target_cluster_config_path> <target_table_name> "
    ...
    try {
      ...
      if (opName.equals("get")) {
        ...
        for (Pair<byte[], byte[]> p : sortKeyValuePairs) {
          System.out.printf(
              "\"%s\":\"%s\"\t\"%s\"\n",
              new String(hashKey), new String(p.getKey()), new String(p.getValue()));
        }
        if (sortKeyValuePairs.size() > 0) System.out.println();
        System.out.printf(
        ...
        if (sortKeys.size() > 0) System.out.println();
        System.out.printf(
        ...
        PegasusScannerInterface scanner =
            client.getScanner(appName, hashKey, startSortKey, stopSortKey, new ScanOptions());
        int count = 0;
        ...
        List<PegasusScannerInterface> scanners =
            client.getUnorderedScanners(appName, 1, new ScanOptions());
        int count = 0;
        if (scanners.size() > 0) {
        ...
        PegasusClientInterface targetClient =
            PegasusClientFactory.createClient(targetClusterConfigPath);
        try {
